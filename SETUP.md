...
- **Figma MCP connection** — planned (2026-09-14) in the *other* direction: a Figma library generated *from* code (`design/tokens.json` → variables, `src/components/` → components with variants), so screens can be redrawn for documentation. Code stays upstream. Needs the Figma connector authorised in claude.ai first. If Figma ever becomes the actual design source instead, revisit `.claude/rules/` for token/component sourcing rules.
