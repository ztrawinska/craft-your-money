...
**Issue:** #8 — done: space (#5, #23), radius (#24, #26), type (P3, 2026-09-17)
**Status 2026-09-17 — type done (P3).** 24 presets on 11 sizes in `@theme` (`--text-<preset>` with line-height, weight and tracking sub-keys; Tailwind's sizes removed), decided in a session with every style rendered current beside proposed (`ds-inspection/plans/2026-09-17-type-scale-session.md`). All 174 `text-[…px]` and 60 `leading-`/`tracking-[…]` uses migrated, product and library; the Price component's currency glyph and decimals are the only raw sizes left, as named parts. `tokens.json` `type` rewritten as the presets (parity test checks values and that every line box is on 4pt); Figma text styles renamed and re-valued, three merged, `Serif/prose` added, Foundations tiles rebuilt; spec §1.4 is the preset table; checker's type section hard-fails.
...
**Status 2026-09-16 — radius done (#24).** `--radius-*: initial` + eight named radii in `@theme`; 44 uses migrated with values unchanged (`rounded-[7px]` → `rounded-button`, the chip family → `rounded-chip`, a true circle stays `rounded-full`); shadcn's `--radius-sm/md/lg/xl` bridge removed; parity test for `--radius-*` ↔ `tokens.json`; checker's radius section hard-fails on any raw value or Tailwind default; spec §1.6 rewritten as the named table, §2 sections point at names; Figma descriptions carry the class names. Finding: `frame` (6px) had no app consumer — folded into `band` when #26 consolidated the set onto 2 · 4 · 8 · 12 · 16 · chip (2026-09-17, one line per radius, reviewed on screenshots first).
