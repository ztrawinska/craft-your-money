 ...
 *
 * The review is scripted here (a live Claude API would drop into generateReview).
 * It's bounded: verdict → three findings → two preview-only scenarios → three
 * follow-up topics, then it stops. Scenarios are shown, never applied.
 */
"use client";
...
export function PriceCheck({ ctx }: { ctx: ReviewContext | null }) {
  ...
  const review = generateReview(ctx, topic);
  ...
  return (
    <div className="mt-5 rounded-r-[6px] border-l-2 border-iris bg-iris/[0.055] py-4 pl-4 pr-4">
      ...
      {/* provenance — Used / Assumed / Can't know */}
